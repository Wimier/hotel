		...
		<scroll-view scroll-y class="order-scroll" @scrolltolower="loadMore">
			...
			<view class="order-card" v-for="order in orderList" :key="order.id" @click="goDetail(order.id)">
				...
				<view class="card-footer">
					...
					<view class="actions">
						<button v-if="order.status === 0" class="btn pay-btn"
							@click.stop="payVirtual(order)">立即支付</button>

						<button v-if="order.status === 3 && order.commentStatus === 0" class="btn comment-btn"
